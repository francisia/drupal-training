...
function coolthing_preprocess_html(&$vars) {
  ...
  if (theme_get_setting('coolthing_inline_field_descriptions')) {
    $vars['classes_array'][] = 'coolthing-inline-field-descriptions';
  }
}
 ...
 */
function coolthing_css_alter(&$css) {
  if (isset($css['modules/overlay/overlay-child.css'])) {
    $css['modules/overlay/overlay-child.css']['data'] = drupal_get_path('theme', 'coolthing') . '/overlay-child.css';
  }
  if (isset($css['modules/shortcut/shortcut.css'])) {
    $css['modules/shortcut/shortcut.css']['data'] = drupal_get_path('theme', 'coolthing') . '/shortcut.css';
  }
}
...
function coolthing_theme() {
  ...
  $items['help_page'] = array(
    ...
    'preprocess functions' => array(
      'template_preprocess',
      'coolthing_preprocess_help_page',
    ),
  ...
  $items['node_form'] = array(
    ...
    'preprocess functions' => array(
      'coolthing_preprocess_form_buttons',
      'coolthing_preprocess_form_node',
    ),
  );
...
function coolthing_preprocess_page(&$vars) {
  ...
  // Process local tasks. This will get called for coolthing and its subthemes.
  _coolthing_local_tasks($vars);
 ...
 */
function coolthing_preprocess_fieldset(&$vars) {
 ...
 */
function coolthing_preprocess_form_buttons(&$vars) {
 ...
 */
function coolthing_preprocess_form_node(&$vars) {
 ...
 */
function coolthing_preprocess_button(&$vars) {
 ...
 */
function coolthing_preprocess_help(&$vars) {
  $vars['hook'] = 'help';
 ...
 */
function coolthing_preprocess_help_page(&$vars) {
  $vars['hook'] = 'help-page';
 ...
 */
function coolthing_preprocess_node(&$vars) {
  $vars['layout'] = TRUE;
  if ($vars['display_submitted']) {
    $vars['submitted'] = _coolthing_submitted($vars['node']);
  }
}
 ...
 */
function coolthing_preprocess_comment(&$vars) {
  $vars['layout'] = TRUE;
  $vars['submitted'] = _coolthing_submitted($vars['comment']);
}

/**
 * Preprocessor for theme('comment_wrapper').
 */
function coolthing_preprocess_comment_wrapper(&$vars) {
  $vars['hook'] = 'box';
  $vars['layout'] = FALSE;
 ...
 */
function coolthing_breadcrumb($vars) {
  $output = '';
 ...
 */
function coolthing_filter_guidelines($variables) {
  return '';
}
 ...
 */
function coolthing_node_add_list($vars) {
  ...
  if ($content) {
    foreach ($content as $item) {
      ...
      if (isset($item['localized_options']['attributes']['class'])) {
        $item['localized_options']['attributes']['class'] += _coolthing_icon_classes($item['href']);
      }
      else {
        $item['localized_options']['attributes']['class'] = _coolthing_icon_classes($item['href']);
      }
 ...
 */
function coolthing_admin_block_content($vars) {
  $content = $vars['content'];
  ...
  if (!empty($content)) {
    ...
    foreach ($content as $k => $item) {
      ...
      if (!empty($content[$k]['localized_options']['attributes']['class'])) {
        $content[$k]['localized_options']['attributes']['class'] += _coolthing_icon_classes($item['href']);
      }
      else {
        $content[$k]['localized_options']['attributes']['class'] = _coolthing_icon_classes($item['href']);
      }
 ...
 */
function coolthing_admin_drilldown_menu_item_link($link) {
  ...
  if (!isset($link['localized_options']['attributes']['class'])) {
    $link['localized_options']['attributes']['class'] = _coolthing_icon_classes($link['href']);
  }
  else {
    $link['localized_options']['attributes']['class'] += _coolthing_icon_classes($link['href']);
  }
 ...
 */
function coolthing_preprocess_textfield(&$vars) {
 ...
 */
function coolthing_menu_local_task($variables) {
 ...
 */
function coolthing_render_clone($elements) {
  static $instance;
  if (!isset($instance)) {
    $instance = 1;
  }
 ...
 */
function _coolthing_submitted($node) {
 ...
 */
function _coolthing_icon_classes($path) {
  $classes = array();
  $args = explode('/', $path);
